...
async function sendImage(sock, jid, imagePathOrUrl, caption = "") {
  try {
    await sock.sendMessage(jid, { image: { url: imagePathOrUrl }, caption });
  } catch (err) {
...
async function sendVideo(sock, jid, videoPathOrUrl, caption = "") {
  try {
    await sock.sendMessage(jid, { video: { url: videoPathOrUrl }, caption });
  } catch (err) {
...
async function downloadMediaMessage(message, outputDir = "./downloads") {
  try {
    ...
    const mimeType = messageType.replace("Message", "");
    ...
    const filename = path.join(outputDir, `${Date.now()}.${mimeType}`);
...
async function autoReply(sock, msg) {
  try {
    ...
    if (["hi", "hello", "hey"].includes(lower)) {
      await sendText(sock, from, "👋 Hey! I'm *CYPHER MD BOT*");
